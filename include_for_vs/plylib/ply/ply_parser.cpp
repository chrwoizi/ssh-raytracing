  ...
  while (std::getline(istream, line)) {
    ...
    if (stringstream.eof()) {
      ...
      if (keyword == "format") {
        ...
        if (!stringstream || !stringstream.eof() || !isspace(space_format_format_string) || !isspace(space_format_string_version)) {
          if (error_callback_) {
            error_callback_(line_number_, "parse error");
          }
          return false;
      ...
      // element
        ...
        if (!stringstream || !stringstream.eof() || !isspace(space_element_name) || !isspace(space_name_count)) {
          if (error_callback_) {
            error_callback_(line_number_, "parse error");
          }
          return false;
        ...
        std::tr1::shared_ptr<element> element_ptr(new element(name, count, boost::get<0>(element_callbacks), boost::get<1>(element_callbacks)));
      ...
      // property
        ...
        if (type_or_list != "list") {
          ...
          if (!stringstream || !isspace(space_list_size_type) || !isspace(space_size_type_scalar_type) || !isspace(space_scalar_type_name)) {
            if (error_callback_) {
              error_callback_(line_number_, "parse error");
            }
            return false;
