#ifndef DISCLIGHT_HPP
#define DISCLIGHT_HPP
...
public:
	DiscLight(const vec& color, const vec& position, float radius, const vec& normal, const vec& binormal, float linearAttenuation = 0.0f, float squaredAttenuation = 0.0f) 
		: Light(color), position(position), radius(radius), linearAttenuation(linearAttenuation), squaredAttenuation(squaredAttenuation),
	...
	{
		// compute x and y
		float r1 = float(2.0 * 3.1415926535 * double(rand()) / double(RAND_MAX));
		float r2 = float(radius * double(rand()) / double(RAND_MAX));
		float sampleX = cos(r1) * r2;
		float sampleY = sin(r1) * r2;
