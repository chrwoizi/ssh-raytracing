...
/**
	...
	std::vector modified to take instances by reference in method "resize". this enables the parametrization with aligned structures
...
// aligned_vector standard header
#pragma once
#ifndef _ALIGNEDVECTOR_
#define _ALIGNEDVECTOR_
#ifndef RC_INVOKED
#include <memory>
#include <stdexcept>
...
template<class _Ty,
	...
	class aligned_vector;
		...
		// TEMPLATE CLASS _AlignedVector_const_iterator
template<class _Ty,
	class _Alloc>
	class _AlignedVector_const_iterator
		: public _Ranit<_Ty, typename _Alloc::difference_type,
			typename _Alloc::const_pointer, typename _Alloc::const_reference>
	{	// iterator for nonmutable aligned_vector
public:
	typedef _AlignedVector_const_iterator<_Ty, _Alloc> _Myt;
	typedef aligned_vector<_Ty, _Alloc> _Myvec;
	...
	// Setting _Inner_type implies that the internal structure of aligned_vector is
	...
	// pass a aligned_vector iterator.
	typedef _Tptr _Inner_type;

	_AlignedVector_const_iterator()
		{	// construct with null pointer
		_Myptr = 0;
		}

 #if _HAS_ITERATOR_DEBUGGING
	_AlignedVector_const_iterator(_Tptr _Ptr, const _Container_base *_Paligned_vector)
		{	// construct with pointer _Ptr
		_SCL_SECURE_VALIDATE(_Paligned_vector == NULL || (((_Myvec *)_Paligned_vector)->_Myfirst <= _Ptr && _Ptr <= ((_Myvec *)_Paligned_vector)->_Mylast));
		this->_Adopt(_Paligned_vector);
		_Myptr = _Ptr;
		}
	...
	_AlignedVector_const_iterator(_Tptr _Ptr, const _Container_base *_Paligned_vector)
		{	// construct with pointer _Ptr
		_SCL_SECURE_VALIDATE(_Paligned_vector != NULL && ((_Myvec *)_Paligned_vector)->_Myfirst <= _Ptr && _Ptr <= ((_Myvec *)_Paligned_vector)->_Mylast);
		this->_Set_container(_Paligned_vector);
		_Myptr = _Ptr;
		}

 #else
	explicit _AlignedVector_const_iterator(_Tptr _Ptr)
		{	// construct with pointer _Ptr
		_Myptr = _Ptr;
		}
	...
	reference operator*() const
		...
		if (this->_Mycont == 0
			...
			{
			_DEBUG_ERROR("aligned_vector iterator not dereferencable");
	...
	_Myt& operator++()
		...
		if (this->_Mycont == 0
			|| ((_Myvec *)this->_Mycont)->_Mylast <= _Myptr)
			_DEBUG_ERROR("aligned_vector iterator not incrementable");
 #endif /* _HAS_ITERATOR_DEBUGGING */
	...
	_Myt& operator--()
		...
		if (this->_Mycont == 0
			|| _Myptr == ((_Myvec *)this->_Mycont)->_Myfirst)
			_DEBUG_ERROR("aligned_vector iterator not decrementable");
 #endif /* _HAS_ITERATOR_DEBUGGING */
	...
	void _Compat(const _Myt& _Right) const
		...
		if (this->_Mycont == 0 || this->_Mycont != _Right._Mycont)
			{
			_DEBUG_ERROR("aligned_vector iterators incompatible");
	...
	static void _Xlen()
		...
		_THROW(length_error, "aligned_vector<T> too long");
		}

	static void _Xran()
		{	// report an out_of_range error
		_THROW(out_of_range, "invalid aligned_vector<T> subscript");
		}

	static void _Xinvarg()
		{	// report an invalid_argument error
		_THROW(invalid_argument, "invalid aligned_vector<T> argument");
		}

	_Tptr _Myptr;	// offset of element in aligned_vector
	};

template<class _Ty,
	class _Alloc> inline
	_AlignedVector_const_iterator<_Ty, _Alloc> operator+(
		typename _AlignedVector_const_iterator<_Ty, _Alloc>::difference_type _Off,
		_AlignedVector_const_iterator<_Ty, _Alloc> _Next)
	{	// add offset to iterator
	return (_Next += _Off);
	}

		// TEMPLATE CLASS _AlignedVector_iterator
template<class _Ty,
	class _Alloc>
	class _AlignedVector_iterator
		: public _AlignedVector_const_iterator<_Ty, _Alloc>
	{	// iterator for mutable aligned_vector
public:
	typedef _AlignedVector_iterator<_Ty, _Alloc> _Myt;
	typedef _AlignedVector_const_iterator<_Ty, _Alloc> _Mybase;
	...
	_AlignedVector_iterator()
		{	// construct with null aligned_vector pointer
		}

 #if _HAS_ITERATOR_DEBUGGING
	_AlignedVector_iterator(pointer _Ptr, const _Container_base *_Paligned_vector)
		: _Mybase(_Ptr, _Paligned_vector)
		{	// construct with pointer _Ptr
		}
 ...
 #else
	explicit _AlignedVector_iterator(pointer _Ptr)
		: _Mybase(_Ptr)
		{	// construct with pointer _Ptr
		}
 #endif /* _HAS_ITERATOR_DEBUGGING */
...
template<class _Ty,
	class _Alloc> inline
	_AlignedVector_iterator<_Ty, _Alloc> operator+(
		typename _AlignedVector_iterator<_Ty, _Alloc>::difference_type _Off,
		_AlignedVector_iterator<_Ty, _Alloc> _Next)
	{	// add offset to iterator
	return (_Next += _Off);
	}

		// TEMPLATE CLASS _AlignedVector_val
template<class _Ty,
	class _Alloc>
	class _AlignedVector_val
		: public _CONTAINER_BASE_AUX_ALLOC<_Alloc>
	{	// base class for aligned_vector to hold allocator _Alval
protected:
	_AlignedVector_val(_Alloc _Al = _Alloc())
	...
	_Alty _Alval;	// allocator object for values
		...
		// TEMPLATE CLASS aligned_vector
template<class _Ty,
	class _Ax>
	class aligned_vector
		: public _AlignedVector_val<_Ty, _Ax>
	{	// varying size array of values
public:
	typedef aligned_vector<_Ty, _Ax> _Myt;
	typedef _AlignedVector_val<_Ty, _Ax> _Mybase;
  ...
  #define _VEC_ITER_BASE(it)	(it)._Myptr
	...
	typedef _AlignedVector_iterator<_Ty, _Alloc> iterator;
	typedef _AlignedVector_const_iterator<_Ty, _Alloc> const_iterator;

//	friend class _AlignedVector_iterator<_Ty, _Alloc>;
	friend class _AlignedVector_const_iterator<_Ty, _Alloc>;
	...
	aligned_vector()
		: _Mybase()
		{	// construct empty aligned_vector
		_Buy(0);
		}

	explicit aligned_vector(const _Alloc& _Al)
		: _Mybase(_Al)
		{	// construct empty aligned_vector with allocator
		_Buy(0);
		}
	...
	aligned_vector(size_type _Count, const _Ty& _Val)
		: _Mybase()
		{	// construct from _Count * _Val
		_Construct_n(_Count, _Val);
		}

	aligned_vector(size_type _Count, const _Ty& _Val, const _Alloc& _Al)
		: _Mybase(_Al)
		{	// construct from _Count * _Val, with allocator
		_Construct_n(_Count, _Val);
		}

	aligned_vector(const _Myt& _Right)
	...
	template<class _Iter>
		aligned_vector(_Iter _First, _Iter _Last, const _Alloc& _Al)
		: _Mybase(_Al)
	...
	const_reference operator[](size_type _Pos) const
		...
		if (size() <= _Pos)
			{
			_DEBUG_ERROR("aligned_vector subscript out of range");
			_SCL_SECURE_OUT_OF_RANGE;
			}
 #endif /* _HAS_ITERATOR_DEBUGGING */
	...
	void pop_back()
		...
		if (empty())
			_DEBUG_ERROR("aligned_vector empty before pop");
		else
			{	// erase last element
	...
	template<class _Iter>
		...
		if (_Where._Mycont != this
			...
			_DEBUG_ERROR("aligned_vector insert iterator outside range");
 #endif /* _HAS_ITERATOR_DEBUGGING */
	...
	iterator erase(const_iterator _Where)
		...
		if (_Where._Mycont != this
			|| _Where._Myptr < _Myfirst || _Mylast <= _Where._Myptr)
			_DEBUG_ERROR("aligned_vector erase iterator outside range");
...
protected:
	...
	// aligned_vector implements a performant swap
template <class _Ty, class _Ax>
	class _Move_operation_category<aligned_vector<_Ty, _Ax> >
	{
	public:
		typedef _Swap_move_tag _Move_cat;
	};

		// aligned_vector TEMPLATE FUNCTIONS
template<class _Ty,
	class _Alloc> inline
	bool operator==(const aligned_vector<_Ty, _Alloc>& _Left,
		const aligned_vector<_Ty, _Alloc>& _Right)
	{	// test for aligned_vector equality
...
template<class _Ty,
	...
	bool operator!=(const aligned_vector<_Ty, _Alloc>& _Left,
		const aligned_vector<_Ty, _Alloc>& _Right)
	{	// test for aligned_vector inequality
	return (!(_Left == _Right));
	}
...
template<class _Ty,
	...
	bool operator<(const aligned_vector<_Ty, _Alloc>& _Left,
		const aligned_vector<_Ty, _Alloc>& _Right)
	{	// test if _Left < _Right for aligned_vectors
...
template<class _Ty,
	class _Alloc> inline
	void swap(aligned_vector<_Ty, _Alloc>& _Left, aligned_vector<_Ty, _Alloc>& _Right)
	{	// swap _Left and _Right aligned_vectors
	_Left.swap(_Right);
	}

//
// TEMPLATE CLASS aligned_vector<bool, Alloc> AND FRIENDS
//

		// CLASS _AlignedVb_iter_base
...
public:
	...
	_AlignedVb_iter_base()
		: _Myptr(0), _Myoff(0)
		{	// construct with null pointer
		}

 #if _HAS_ITERATOR_DEBUGGING
	_AlignedVb_iter_base(_Vbase *_Ptr, _Sizet _Off,
	...
	static void _Xlen()
		{	// report a length_error
		_THROW(length_error, "aligned_vector<bool> too long");
			}

	static void _Xran()
		{	// report an out_of_range error
		_THROW(out_of_range, "invalid aligned_vector<bool> subscript");
		}

	static void _Xinvarg()
		{	// report an invalid_argument error
		_THROW(invalid_argument, "invalid aligned_vector<bool> argument");
		}
	...
	_Sizet _My_actual_offset() const
		...
		// CLASS _AlignedVb_reference
template<class _Sizet,
	class _Difft,
	class _MycontTy>
	class _AlignedVb_reference
		: public _AlignedVb_iter_base<_Sizet, _Difft, _MycontTy>
	{	// reference to a bit within a base word
public:
	typedef _AlignedVb_iter_base<_Sizet, _Difft, _MycontTy> _Mybase;
	typedef _AlignedVb_reference<_Sizet, _Difft, _MycontTy> _Mytype;

	_AlignedVb_reference()
		{	// construct with null pointer
		}

 #if _HAS_ITERATOR_DEBUGGING || _SECURE_SCL
	_AlignedVb_reference(const _Mybase& _Right)
	...
	_Mytype& operator=(const _Mytype& _Right)
		{	// assign _AlignedVb_reference _Right to bit
	...
	_Vbase *_Getptr() const
		...
		if (this->_Mycont == 0 || this->_Myptr == 0)
			{
			_DEBUG_ERROR("aligned_vector<bool> iterator not dereferencable");
			_SCL_SECURE_OUT_OF_RANGE;
			}
 #else /* _HAS_ITERATOR_DEBUGGING */
...
template<class _Sizet,
	class _Difft,
	class _MycontTy>
	void swap(_AlignedVb_reference<_Sizet, _Difft, _MycontTy> _Left,
		_AlignedVb_reference<_Sizet, _Difft, _MycontTy> _Right)
	{	// swap _Left and _Right aligned_vector<bool> elements
	bool _Val = _Left;
	_Left = _Right;
	_Right = _Val;
	}

		// CLASS _AlignedVb_const_iterator
template<class _Sizet,
	class _Difft,
	class _MycontTy>
	class _AlignedVb_const_iterator
		: public _AlignedVb_iter_base<_Sizet, _Difft, _MycontTy>
	{	// iterator for nonmutable aligned_vector<bool>
public:
	typedef _AlignedVb_iter_base<_Sizet, _Difft, _MycontTy> _Mybase;
	typedef _AlignedVb_const_iterator<_Sizet, _Difft, _MycontTy> _Mytype;

	typedef _AlignedVb_reference<_Sizet, _Difft, _MycontTy> _Reft;
	...
	_AlignedVb_const_iterator()
		{	// construct with null reference
		}

 #if _HAS_ITERATOR_DEBUGGING || _SECURE_SCL
	_AlignedVb_const_iterator(const _Vbase *_Ptr, const _Container_base *_Mypvbool)
		: _Mybase((_Vbase *)_Ptr, 0, (_Container_base *)_Mypvbool)

 #else
	_AlignedVb_const_iterator(const _Vbase *_Ptr)
...
protected:
	...
	void _Compat(const _Mytype& _Right) const
		{	// test for compatible iterator pair
		if (this->_Mycont == 0 || this->_Mycont != _Right._Mycont)
			_DEBUG_ERROR("aligned_vector<bool> iterators incompatible");
		}
 #endif /* _HAS_ITERATOR_DEBUGGING */
...
template<class _Sizet,
	class _Difft,
	class _MycontTy>
	_AlignedVb_const_iterator<_Sizet, _Difft, _MycontTy> operator+(_Difft _Off,
		_AlignedVb_const_iterator<_Sizet, _Difft, _MycontTy> _Right)
		{	// return _Right + integer
		return (_Right += _Off);
		}

	// CLASS _AlignedVb_iterator
template<class _Sizet,
	class _Difft,
	class _MycontTy>
	class _AlignedVb_iterator
		: public _AlignedVb_const_iterator<_Sizet, _Difft, _MycontTy>
	{	// iterator for mutable aligned_vector<bool>
public:
	typedef _AlignedVb_const_iterator<_Sizet, _Difft, _MycontTy> _Mybase;
	typedef _AlignedVb_iterator<_Sizet, _Difft, _MycontTy> _Mytype;
 ...
 #if _HAS_ITERATOR_DEBUGGING || _SECURE_SCL
	_AlignedVb_iterator(_Vbase *_Ptr, _Container_base *_Mypvbool)
		: _Mybase(_Ptr, _Mypvbool)

 #else
	_AlignedVb_iterator( _Vbase *_Ptr)
		: _Mybase(_Ptr)
 #endif /* _HAS_ITERATOR_DEBUGGING */
...
template<class _Sizet,
	class _Difft,
	class _MycontTy>
	_AlignedVb_iterator<_Sizet, _Difft, _MycontTy> operator+(_Difft _Off,
		_AlignedVb_iterator<_Sizet, _Difft, _MycontTy> _Right)
		{	// return _Right + integer
		return (_Right += _Off);
		}

		// CLASS aligned_vector_bool
template<class _Alloc>
	class aligned_vector<_Bool, _Alloc>
		: public _CONTAINER_BASE_AUX_ALLOC<_Alloc>
	{	// varying size array of bits
public:
	typedef typename _Alloc::size_type size_type;
	typedef typename _Alloc::difference_type _Dift;
	typedef std::aligned_vector<_Vbase,
		typename _Alloc::template rebind<_Vbase>::other>
			_Vbtype;
	typedef std::aligned_vector<_Bool, _Alloc> _Myt;
	...
	typedef _AlignedVb_reference<size_type, _Dift, _Myt> reference;
	typedef bool const_reference;
	typedef bool value_type;

	typedef reference _Reft;
	typedef _AlignedVb_const_iterator<size_type, difference_type, _Myt> const_iterator;
	typedef _AlignedVb_iterator<size_type, difference_type, _Myt> iterator;

	friend class _AlignedVb_iter_base<size_type, difference_type, _Myt>;
	friend class _AlignedVb_reference<size_type, difference_type, _Myt>;
	friend class _AlignedVb_const_iterator<size_type, difference_type, _Myt>;
	friend class _AlignedVb_iterator<size_type, difference_type, _Myt>;
	...
	aligned_vector(const _Myt& _Right)
		...
		{	// copy construct aligned_vector; an implicitly defined copy constructor would not create an aux object.
		}
	...
	explicit aligned_vector(size_type _Count, bool _Val = false)
		: _CONTAINER_BASE_AUX_ALLOC<_Alloc>(_Alloc()), _Mysize(0), _Myvec(_Nw(_Count), (_Vbase)(_Val ? -1 : 0))
		{	// construct from _Count * _Val
		_Trim(_Count);
		}

	aligned_vector(size_type _Count, bool _Val, const _Alloc& _Al)
 ...
 #if _HAS_ITERATOR_DEBUGGING
		if (end() <= _Where)
			_DEBUG_ERROR("aligned_vector<bool> erase iterator outside range");
...
protected:
 ...
 #if _HAS_ITERATOR_DEBUGGING
		if (end() < _Where)
			_DEBUG_ERROR("aligned_vector<bool> insert iterator outside range");
	...
	void _Orphan_range(size_type _Offlo, size_type _Offhi) const
		{	// orphan iterators within specified (inclusive) range
		typedef _AlignedVb_iter_base<size_type, difference_type, _Myt> _Myiterbase;
		...
		while (*_Pnext != 0)
			{	// test offset from beginning of aligned_vector
	...
	void _Trim(size_type _Size)
		{	// trim base aligned_vector to exact length in bits
		if (max_size() < _Size)
			_Xlen();	// result too long
	...
	_Vbtype _Myvec;	// base aligned_vector of words
	};

typedef aligned_vector<bool, allocator<bool> > _Baligned_vector;

#if _HAS_TRADITIONAL_STL
 typedef _Baligned_vector bit_aligned_vector;
 #define __aligned_vector__	aligned_vector
